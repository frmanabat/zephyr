...
void max86178_flush_fifo(const struct device *dev)\
{
    ...
    uint8_t fifo_config = FIELD_PREP(MAX86178_FIFO_CFG2_FLUSH_FIFO_MSK, 1) |
...
void max86178_submit_stream(const struct device *dev, struct rtio_iodev_sqe *iodev_sqe)
{
    ...
    if (fifo_watermark_irq != data->fifo_watermark_irq) {
        ...
        ret = max86178_reg_update(dev, cfg_178->route_to_int2 ? MAX86178_INT2_EN1 : MAX86178_INT1_EN1, MAX86178_INT_EN1_A_FULL_MSK, fifo_watermark_irq);
        if (ret < 0) {
...
static void max86178_complete_op_cb(struct rtio *r, const struct rtio_sqe *sqe, int result, void *arg)
{
    ARG_UNUSED(result);
	const struct device *dev = (const struct device *)arg;
	const struct max86178_dev_config *config = (const struct max86178_dev_config *)dev->config;
...
static void max86178_process_fifo_samples_cb(struct rtio *r, const struct rtio_sqe *sqe, int result, void *arg)
{
    ARG_UNUSED(result);
    const struct device *dev = (const struct device *)arg;
    struct max86178_data *data = (struct max86178_data *)dev->data;
    ...
    uint16_t fifo_sample_count = (FIELD_GET(MAX86178_FIFO_COUNTER1_DATA_COUNT_MSB, data->fifo_counter[0]) << 8) | data->fifo_counter[1];
    ...
    LOG_DBG("Requesting buffer [%u, %u] got %u", (unsigned int)min_read_size, (unsigned int)ideal_read_size, buf_len);
...
#endif
    rtio_sqe_prep_callback(complete_op, max86178_complete_op_cb, (void *)dev, (void *)current_sqe);
    rtio_submit(data->rtio_ctx, 0);
}

static void max86178_process_status1_cb(struct rtio *r, const struct rtio_sqe *sqe, int result, void *arg)
{
    ARG_UNUSED(result);
    const struct device *dev = (const struct device *)arg;
    struct max86178_data *data = (struct max86178_data *)dev->data;
    ...
    if (data_opt == SENSOR_STREAM_DATA_NOP || data_opt == SENSOR_STREAM_DATA_DROP) {
        ...
        data->sqe = NULL;
        if(rtio_sqe_rx_buf(current_sqe, sizeof(struct max86178_fifo_data), sizeof(struct max86178_fifo_data), &buf, &buf_len) != 0) {
            rtio_iodev_sqe_err(current_sqe, -ENOMEM);
            gpio_pin_interrupt_configure_dt(&config->interrupt_gpio, GPIO_INT_EDGE_TO_ACTIVE);
            return;
        }
...
#endif
    write_fifo_addr->flags |= RTIO_SQE_TRANSACTION;
    rtio_sqe_prep_read(read_fifo_addr, data->iodev, RTIO_PRIO_NORM, data->fifo_counter, 2, NULL);
    read_fifo_addr->flags |= RTIO_SQE_CHAINED;
#ifdef CONFIG_I2C_RTIO
    read_fifo_addr->iodev_flags |= RTIO_IODEV_I2C_STOP | RTIO_IODEV_I2C_RESTART;
#endif
