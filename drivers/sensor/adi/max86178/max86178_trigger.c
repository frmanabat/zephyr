...
static void max86178_thread_cb(const struct device *dev)
{
	...
	if ((data->ppg_thresh1_hilo_handler != NULL) &&
	    (status[0] & MAX86178_STATUS1_PPG_THRESH1_HILO_MSK)) {
		data->ppg_thresh1_hilo_handler(dev, data->ppg_thresh1_hilo_trigger);
	}

	if ((data->ppg_thresh2_hilo_handler != NULL) &&
	    (status[0] & MAX86178_STATUS1_PPG_THRESH2_HILO_MSK)) {
	...
	if ((data->fifo_data_rdy_handler != NULL) &&
	    (status[0] & MAX86178_STATUS1_FIFO_DATA_RDY_MSK)) {
		data->fifo_data_rdy_handler(dev, data->fifo_data_rdy_trigger);
	}

	if ((data->ppg_frame_rdy_handler != NULL) &&
	    (status[0] & MAX86178_STATUS1_PPG_FRAME_RDY_MSK)) {
	...
	if ((data->invalid_ppg_cfg_handler != NULL) &&
	    (status[1] & MAX86178_STATUS2_INVALID_PPG_CFG_MSK)) {
	...
	if ((data->phase_unlock_handler != NULL) &&
	    (status[2] & MAX86178_STATUS3_PHASE_UNLOCK_MSK)) {
	...
	if ((data->ecg_fast_rec_handler != NULL) &&
	    (status[3] & MAX86178_STATUS4_ECG_FAST_REC_MSK)) {
	...
	if ((data->bioz_loff_nl_handler != NULL) &&
	    (status[4] & MAX86178_STATUS5_BIOZ_LOFF_NL_MSK)) {
		data->bioz_loff_nl_handler(dev, data->bioz_loff_nl_trigger);
	}

	if ((data->bioz_loff_nh_handler != NULL) &&
	    (status[4] & MAX86178_STATUS5_BIOZ_LOFF_NH_MSK)) {
		data->bioz_loff_nh_handler(dev, data->bioz_loff_nh_trigger);
	}

	if ((data->bioz_loff_pl_handler != NULL) &&
	    (status[4] & MAX86178_STATUS5_BIOZ_LOFF_PL_MSK)) {
		data->bioz_loff_pl_handler(dev, data->bioz_loff_pl_trigger);
	}

	if ((data->bioz_loff_ph_handler != NULL) &&
	    (status[4] & MAX86178_STATUS5_BIOZ_LOFF_PH_MSK)) {
		data->bioz_loff_ph_handler(dev, data->bioz_loff_ph_trigger);
	}

	if ((data->bioz_drvp_off_handler != NULL) &&
	    (status[4] & MAX86178_STATUS5_BIOZ_DRV_OOR_MSK)) {
...
static void max86178_gpio_callback(const struct device *dev, struct gpio_callback *cb,
				   uint32_t pins)
{
	int ret;
